...
class GA_TSP(GeneticAlgorithmBase):
    ...
    def crtbp(self):
        ...
        if self.case_type == "grade":
            self.Chrom = np.random.randint(0, 100, [self.size_pop, self.len_chrom])

        elif self.case_type == "smallest":
            self.Chrom = np.random.randint(-5, 30, [self.size_pop, self.len_chrom])
        elif self.case_type == "median":
            self.Chrom = np.random.randint(-2, 2, [self.size_pop, self.len_chrom])
        elif self.case_type == "checksum":
            self.Chrom = np.array([[chr(i) for i in row] for row in np.random.randint(33, 127, [self.size_pop, self.len_chrom])])
        elif self.case_type == "digits":
            self.Chrom = np.random.randint(0, 10, [self.size_pop, self.len_chrom])
        elif self.case_type == "syllables":
            self.Chrom = np.array([[chr(i) for i in row] for row in np.random.randint(33, 127, [self.size_pop, self.len_chrom])])
